						...
						for (int i = xlb; i <= xub; i++) {
							...
							if (interaction_pair) {
								if (!poles(i, j, k).is_leaf || !poles(i0, j0, k0).is_leaf) {
									...
									l1->phi_dot() += d0 * dm2;
									dm2 *= dxinv;
									l1->phi_dot(0) += d1x * dm2;
									l1->phi_dot(1) += d1y * dm2;
									l1->phi_dot(2) += d1z * dm2;
								}
							}
						}
					}
			...
			for (int j = FBW; j < FNX - FBW; j++) {
				for (int i = FBW; i < FNX - FBW; i++) {
					if (g0->poles(i, j, k).is_leaf) {
						...
						lz += (g0->_4force(i, j, k).g[1] * g0->poles(i, j, k).X[0] - g0->_4force(i, j, k).g[0] * g0->poles(i, j, k).X[1]) * g0->poles(i, j, k).M();
