...
! integrate the iso7 network
       call netint(beg,stptry,stpmin,tend,0.0d0,ys2, &
                   tol,neqs,nok,nbad,kount,odescal, &
                   iso7,siso7,biso7,stifbs_ma28,fail)

       if( fail .eq. 1 ) then
           write(*,*) 'Retrying netint'
           one_step = .false.
           expansion = .true.
           stptry = max(beg * 1.0d-10,1.0d-16)
           stpmin = stptry * 1.0d-12
...
!---------------------------------------------------------------------
      subroutine netint(start,stptry,stpmin,stopp,stpsav,bc, &
                        ...
                        derivs,jakob,bjakob,steper,fail)
      include 'implno.dek'
      include 'burn_common.dek'
      include 'network.dek'
...
! declare the pass
      ...
      double precision start,stptry,stpmin,stopp,stpsav,bc(ylogi),eps,odescal
...
! get the composition
 ...
 20    write(6,01) 'give initial composition:'
       ...
       if (ictype .eq. 3) then
        write(6,01) &
        'n h1 he4 c12 c13 n14 o16 ne20 ne22 mg24 si28 fe52 fe54 fe56 ni56 =>'
...
!       write(6,*) k,h
       ...
       if (xnew .eq. x) then
          hdid = 0.0
          write(*,*) 'step too small in routine stifbs_ma28'
          ierr = -1
          return
       endif
